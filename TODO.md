...
- [ ] slice-014-human-qa-sweep — full core-flow QA in network-enabled environment (token load by ID, wallet/ENS lookup, animation playback, `.glb` export). Blocked on API-backed runtime access: static `python -m http.server` returns 404 for `/api/friendsiesTokens`, and `npx vercel dev` fails in this environment with missing Vercel credentials (`No existing credentials found`) before API routes can boot (reconfirmed 2026-02-18 ~06:45 CT, ~07:02 CT).
